...
const totalRows = parseCsv(totalCsvRaw);
...
// `${seriesID}|${teamID}|${position}` -> { matchID -> [player rows for that team+position in that game] }
...
const suffixAppliesToRow = (subtitleKey, row) => {
  switch (subtitleKey) {
...
const computePlayerGameScore = (row, statSlots, titleKey, subtitleKey) => {
  let total = 0;
  ...
  if (subtitleKey && suffixAppliesToRow(subtitleKey, row)) bonusPercent += subtitlesInfo[subtitleKey]?.percent || 0;
...
const computeSeriesRoleScore = (key, statSlots, titleKey, subtitleKey) => {
  ...
  const gameScores = Object.values(gamesByMatch).map(playerRows => {
    const perPlayerScores = playerRows.map(row => computePlayerGameScore(row, statSlots, titleKey, subtitleKey));
      ...
      <div className='p-40 pb-10 relative w-full bg-gray-950 min-h-screen flex flex-col gap-12'>
        <div className='flex justify-end gap-4 absolute top-8 right-8'>
          <button
            ...
            onClick={() => downloadCsv(totalCsvUrl, 'total')}>
            Download total.csv
          </button>
          <button
            className='text-white bg-purple-900 hover:bg-purple-800 rounded-md px-4 py-2 text-sm cursor-pointer'
            onClick={() => downloadCsv(fantasyScoreCsvUrl, 'fantasy_score')}>
            Download fantasy_score.csv
          </button>
        </div>
        {isHIW && (
